...
def insert_data():
    ...
    company = Company()
    company.name = 'Star Gym S.A.'
    company.ruc = '0000'
    company.address = 'Calle 17 de Obrajes'
    company.mobile = '71570989'
    company.phone = '000'
    company.email = 'mariofrevollo@hotmail.com'
    company.website = ''
    company.description = 'Servicio de gimnasio '
    company.iva = 12.00
    company.save()
    ...
    Product(name='FORMATEO DE COMPUTADORAS', category=category,
            is_service=True, with_tax=False, pvp=15.00, code='FORMATEO85451').save()
    ...
    with open(f'{settings.BASE_DIR}/deploy/json/customers.json', encoding='utf8') as json_file:
        ...
        for item in data[0:20]:
            ...
            client.birthdate = date(random.randint(
                1969, 2006), random.randint(1, 12), random.randint(1, 28))
    ...
    for i in range(1, random.randint(6, 10)):
        ...
        for d in range(1, 8):
            list_products = list(Product.objects.filter(
                stock__gt=0).values_list(flat=True))
            if len(list_products):
                detail = SaleDetail()
                detail.sale_id = sale.id
                detail.product_id = random.choice(list_products)
                detail.cant = random.randint(
                    1, int((detail.product.stock * 0.30)))
